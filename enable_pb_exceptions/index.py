...
from utils.util import (
    get_secret, get_account_by_project_id_gql, get_aws_service_map,
    validate_action_format, update_account_pb_exception_gql, update_account_pb
)
...
def handler(event, context):
    ...
    try:
        ...
        svc_map = get_aws_service_map()
        iterator = svc_map.keys()
        validate_action_errors = []
        for action in exception_actions:
            if validate_action_format(action=action):
                svc, operation = action.split(":")
                for service_name in iterator:
                    if svc_map[service_name].get("StringPrefix", "") == svc:
                        if operation != "*":
                            svc_actions = svc_map[service_name].get("Actions", [])
                            if operation not in svc_actions:
                                err_msg = (
                                    f"Operation '{operation}' not found in valid "
                                    f"'Actions' for service '{service_name}'."
                                )
                                validate_action_errors.append(err_msg)
                        else:
                            logger.info("Operation is '*'.")
                        break
                else:
                    err_msg = f"Service '{svc}' not found in AWS Service Map."
                    validate_action_errors.append(err_msg)
            else:
                err_msg = f"Action '{action}' should be of the format 'service_prefix:Operation'"
                validate_action_errors.append(err_msg)
        if validate_action_errors:
            msg = f"Invalid Action. Details - '{validate_action_errors}'"
            logger.error(msg)
            return {
                "statusCode": 400,
                "isBase64Encoded": False,
        ...
        if account_details:
            ...
            for action in exception_actions:
                if action not in pb_exceptions:
                    logger.info(f"Adding action '{action}' to account 'pb_exceptions'.")
                    pb_exceptions.append(action)
